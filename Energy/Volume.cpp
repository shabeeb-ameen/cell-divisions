...
int Volume::updatePressure() {
    for (auto cell : run_->cells_) {
        ...
        //**EMPTY CELL CHECK**//
        if (cell->type_ == 0) {
            cell->pressure_ = (-1.0) * 2.0 * empty_cell_kv_ * (cell->volume_ - empty_cell_v0_);
        } else {
            cell->pressure_ = (-1.0) * 2.0 * cell->kv_ * (cell->volume_ - cell->v0_);
        }
    }
...
int Volume::updateEnergy() {
    ...
    for (auto cell : run_->cells_) {
        ...
        energy_ += cell->kv_*pow(cell->volume_-cell->v0_, 2.0);
    }
