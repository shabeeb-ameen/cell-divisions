...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        dumpVTK(vertices, polygons, filename="/data/sample.vtk")
        dumpEmptyVTK(vertices, polygons, filename="/data/empty.vtk")
        dumpTopo(vertices, edges, polygons, cells, filename="/data/sample.topo")
...
def dumpVTK(vertices, polygons, filename="sample.vtk"):
...
def dumpEmptyVTK(vertices, polygons, filename="empty.vtk"):
    with open(filename, "w") as file:
        file.write("# vtk DataFile Version 2.0\npolydata\nASCII\nDATASET POLYDATA\n")
        file.write("POINTS {:d} double\n".format(len(vertices)))
        vertices = dict(sorted(vertices.items(), key=lambda item: item[1].id_))
...
def dumpTopo(vertices, edges, polygons, cells, filename="sample.topo"):
    with open(filename, "w") as file:
