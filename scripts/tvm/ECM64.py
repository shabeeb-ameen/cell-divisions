...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        dumpTopo(phantomVertices, fibers, filename="/data/ECM.topo")
        dumpVTK(phantomVertices, fibers, l0, lk, filename="/data/ECM.vtk")
...
def dumpVTK(phantomVertices, fibers, l0, lk, filename="ECM.vtk"):
    edges = []
    for fiber in fibers:
        for edge in fiber:
            edges.append(edge)
...
def dumpTopo(phantomVertices, fibers, filename="ECM.topo"):
    nPhantomVertices = 0
    for pVertex in phantomVertices:
        if pVertex.on:
            nPhantomVertices += 1
