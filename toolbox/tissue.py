...
class Sample:
    ...
    def write_topo(self,filename):
        with open(self.config_dir_+filename, "w") as file:
            file.write(f"vertices {len(self.vertices_):d}\n")
            for _, vertex in self.vertices_.items():
                file.write(f"{vertex.id_:6d} {vertex.position_[0]:12.5e} {vertex.position_[1]:12.5e} {vertex.position_[2]:12.5e}\n")
            file.write("edges {:d}\n".format(len(self.edges_)))
            for _, edge in self.edges_.items():
                file.write(f"{edge.id_:d}")
                for vertexID in edge.vertices_:
                    file.write(f" {vertexID:6d}")
                file.write("\n")            
            file.write(f"polygons {len(self.polygons_):d}\n")
            for _,polygon in self.polygons_.items():
                file.write(f"{polygon.id_:d}")
                for edgeID in polygon.edges_:
                    file.write(f" {edgeID:6d}")
                file.write("\n")
            file.write(f"cells {len(self.cells_):d}\n")
            for _,cell in self.cells_.items():
